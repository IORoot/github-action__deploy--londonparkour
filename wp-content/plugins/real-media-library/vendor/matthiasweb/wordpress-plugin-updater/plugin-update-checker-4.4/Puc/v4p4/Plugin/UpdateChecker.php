...
if (!class_exists('Puc_v4p4_Plugin_UpdateChecker', false)):
    ...
    class Puc_v4p4_Plugin_UpdateChecker extends Puc_v4p4_UpdateChecker {
        ...
        protected function createScheduler($checkPeriod) {
            $scheduler = new Puc_v4p4_Scheduler($this, $checkPeriod, ['load-plugins.php']);
        ...
        protected function getPluginHeader() {
            if (!is_file($this->pluginAbsolutePath)) {
                ...
                $this->triggerError(
                    sprintf("Can't to read the plugin header for '%s'. The file does not exist.", $this->pluginFile),
                    E_USER_WARNING
                );
                return [];
            }
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
            $isRelevant =
                $pluginFile == $this->pluginFile || (!empty($this->muPluginFile) && $pluginFile == $this->muPluginFile);
        ...
        public function displayManualCheckResult() {
            if (isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && $_GET['puc_slug'] == $this->slug) {
                ...
                if ($status == 'no_update') {
                    ...
                    $message = sprintf(
                        _x('A new version of the %s plugin is available.', 'the plugin title', 'plugin-update-checker'),
                        $title
                    );
                } elseif ($status === 'error') {
                    $message = sprintf(
