    ...
    class LicenseUIController {
        ...
        public static function init() {
            ...
            add_filter(
                ...
                '\MatthiasWeb\WPU\V4\LicenseUIController::pluginActionLinksHook',
                10,
                2
            );
        }
        ...
        public static function adminEnqueueScriptsHook($page) {
            if ($page === 'plugins.php') {
                if (!wp_script_is('vue-2.5.16', 'registered')) {
                    $vueUrl = plugin_dir_url(
                        realpath(trailingslashit(__DIR__) . '../../assets/js/' . static::$VUE_FILE)
                    );
                    wp_register_script('vue-2.5.16', $vueUrl . static::$VUE_FILE, [], static::$VUE_VERSION);
                }
        ...
        public static function afterPluginRowHook($pluginFile) {
            ...
            $checkForUpdatesUrl = wp_nonce_url(
                add_query_arg(
                    ['puc_check_for_updates' => 1, 'puc_slug' => $client->config->slug],
            ...
            <script type="text/javascript">
                ...
                (function($, data) {
                    <?php echo file_get_contents(
                        __DIR__ . '../../../assets/js/credentials' . (WP_DEBUG ? '' : '-transpiled') . '.js'
                    ); ?>
        ...
        public static function pluginActionLinksHook($actions, $pluginFile) {
            ...
            if (!LicenseManager::hasLicense($client->config->slug)) {
                $actions['wpls-enter-license'] =
                    '<a href="#" id="enter-license-' .
                    $client->config->slug .
                    '">License Settings' .
                    $noScriptTag .
                    '</a>';
                ...
                $actions['wpls-enter-license'] =
                    '<a style="color: #3db634;" href="#" id="enter-license-' .
                    $client->config->slug .
                    '">Enter License' .
                    $noScriptTag .
                    '</a>';
